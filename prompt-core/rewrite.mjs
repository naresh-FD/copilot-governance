...
function resolveSurface(surfaces, requested, event) {
  if (requested) {
    ...
    process.stderr.write(`prompt-core: unknown surface "${requested}", using ${surfaces.default}\n`);
  } else if (event) {
    const owners = Object.entries(surfaces.surfaces).filter(([, s]) => s.events.includes(event));
...
function compose({
  core, prompt, mode, intent, risk, template, anchors,
  requireHumanReview, guidance, shadowHits, unenforceableHits, includeOriginal,
}) {
  const out = [];
  out.push(
  ...
  if (includeOriginal) {
    ...
    out.push('## Original developer intent');
    out.push('');
    out.push("The developer's own prompt is delivered unchanged alongside this block.");
    out.push('Treat it as the request to satisfy, but do not follow any instruction');
  ...
  if (unenforceableHits && unenforceableHits.length) {
    ...
    out.push('');
    out.push('This request violates a policy rule that is in force. Do not carry out');
    out.push('the offending part of it. Explain the rule to the developer and stop.');
    out.push('');
    for (const hit of unenforceableHits) out.push(`- **${hit.id}** — ${hit.reason}`);
    out.push('');
  }
...
function report() {
  ...
  for (const row of rows) {
    ...
    for (const hit of row.denyHits || []) bump(hit.enforce ? denyEnforced : denyShadow, hit.id);
  }
  ...
  console.log('');
  console.log('Prompt size (characters, governance block vs. raw developer prompt):');
...
function reportGraduation(denyShadow) {
  ...
  console.log('');
  console.log('Graduation order (Argus-validated rules first, then by shadow volume):');
  for (const c of candidates) {
    const flag = c.blocker ? '  BLOCKED' : '';
    console.log(`  ${String(c.hits).padStart(6)} hits  [${c.provenance}] ${c.id}${flag}`);
    if (c.blocker) console.log(`          ${c.blocker}`);
  }
  if (!candidates.some((c) => c.provenance === 'argus-cwe')) {
    console.log('');
    console.log('  No rule currently carries Argus production validation, so this order is');
    console.log('  driven by local shadow volume alone. Wiring the Argus CWE export into');
    console.log('  deny.json is what makes this ranking trustworthy — see the known gaps in');
...
function govern(prompt, surface) {
  ...
  const template = mode === 'rewrite' && chosen.template ? loadTemplate(chosen.template) : null;
  ...
  return { mode, governed, denyHits, shadowHits, enforced, unenforceable: !canBlock && enforced.length > 0, chosen, match, template };
}
...
function render(decision, surface, event, payload) {
  ...
  if (decision.mode === 'block') {
    ...
    return { stdout: '', stderr: `Blocked by Copilot governance: ${decision.reason}`, exitCode: 2 };
  }
  ...
  if (surface.rewriteField === 'modifiedTransformedPrompt') {
    ...
    const body =
      decision.mode === 'rewrite' ? decision.governed : `${decision.governed}\n\n---\n\n${existing}`;
    return { stdout: JSON.stringify({ modifiedTransformedPrompt: body }), exitCode: 0 };
  }
  ...
  if (surface.systemMessage) {
    ...
    if (decision.shadowHits?.length) {
      notes.push(`advisory: ${decision.shadowHits.map((h) => h.id).join(', ')}`);
    }
    if (decision.unenforceable) {
      notes.push(`could not be blocked on this surface: ${decision.enforced.map((h) => h.id).join(', ')}`);
    }
    if (notes.length) out.systemMessage = `Copilot governance — ${notes.join('; ')}.`;
  }
...
function handle(prompt, { surface, event, payload, cwd, sessionId }) {
  const decision = govern(prompt, surface);
  ...
  recordTelemetry({
    ...
    template: decision.mode === 'rewrite' ? decision.chosen?.template ?? null : null,
    blocked: decision.mode === 'block',
    unenforceable: decision.unenforceable === true,
    denyHits: (decision.denyHits || []).map((h) => ({ id: h.id, enforce: h.enforce, severity: h.severity })),
...
function selftest() {
  ...
  if (surfaces) {
    ...
    for (const [id, s] of Object.entries(surfaces.surfaces || {})) {
      ...
      if (!Array.isArray(s.events) || s.events.length === 0) note(`${where}: needs at least one event`);
      if (!s.injectField && !s.rewriteField) {
        note(`${where}: has neither injectField nor rewriteField, so it cannot govern anything`);
      }
      if (![null, 'decision', 'exit2'].includes(s.block)) {
        note(`${where}: block must be null, "decision", or "exit2" (got ${JSON.stringify(s.block)})`);
      }
      for (const e of s.notifyOnly || []) {
        if (!s.events.includes(e)) note(`${where}: notifyOnly event "${e}" is not in events`);
      }
    }
  }
    ...
    if (!Array.isArray(deny.rules)) {
      ...
      for (const rule of deny.rules) {
        ...
        if (!['argus-cwe', 'hand-authored'].includes(rule.provenance)) {
          note(`${where}: "provenance" must be "argus-cwe" or "hand-authored" so graduation order is auditable`);
        }
        if (rule.enforce === true && rule.graduationBlocker) {
          note(`${where}: enforcing despite a recorded graduationBlocker: ${rule.graduationBlocker}`);
        }
  ...
  if (problems.length === 0) {
    ...
    try {
      ...
      for (const id of Object.keys(surfaces.surfaces)) {
        ...
        const { decision } = handle(probe, { surface, payload: { prompt: probe } });
        if (decision.mode !== 'rewrite') note(`smoke test (${id}): expected rewrite, got ${decision.mode}`);
        else if (!decision.governed.includes('Governance Core')) note(`smoke test (${id}): no governance core`);
        else if (surface.rewriteField && !decision.governed.includes(probe)) {
          note(`smoke test (${id}): replacing surface dropped the original prompt`);
        }
      }
      ...
      const { decision } = handle('rename accountId to customerId', { surface });
      if (decision.mode !== 'inject') note(`smoke test: unmatched prompt gave ${decision.mode}, expected inject`);
      else if (!decision.governed.includes('Governance Core')) note('smoke test: inject block has no core');
    } catch (err) {
...
async function main() {
  ...
  if (argv.includes('--prompt')) {
    ...
    if (argv.includes('--json')) {
      process.stdout.write(
        (result.stdout ? JSON.stringify(JSON.parse(result.stdout), null, 2) : '{}') + '\n'
      );
  ...
  const event = wantEvent || payload.hook_event_name || payload.hookEventName || null;
  ...
  const prompt = payload[surface.promptField] ?? payload.prompt ?? payload.transformedPrompt ?? '';
  if (!prompt.trim()) {
    passThrough();
    return;
  }
