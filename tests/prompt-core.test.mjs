...
function runCli(prompt, extraEnv = {}, surface = 'vscode') {
  const out = execFileSync(process.execPath, [ENGINE, '--surface', surface, '--prompt', prompt, '--json'], {
    encoding: 'utf8',
    env: { ...ENV, ...extraEnv },
  });
...
function runHookRaw(surface, prompt, extraEnv = {}, event = 'UserPromptSubmit') {
  const payload = surface === 'copilot-cli'
    ? { hook_event_name: event, transformedPrompt: prompt, prompt, cwd: '.', sessionId: 'test' }
    : { hook_event_name: event, prompt, cwd: '.', session_id: 'test' };
  const res = spawnSync(process.execPath, [ENGINE, '--surface', surface, '--event', event], {
...
test('high-risk intents demand human security review', () => {
  const governed = blockOf(runCli('fix the SQL injection in the account lookup'));
...
test('an unmatched prompt is injected, not rewritten, and is never emptied', () => {
  ...
  assert.ok(body.includes('Governance Core'), 'the generic wrapper must still carry the core');
  assert.match(body, /## Governed approach/, 'no-match must still get the generic governed wrapper');
  assert.ok(!body.includes('## Governed workflow'), 'no workflow should be inlined without a match');
  ...
  assert.ok(body.trimEnd().endsWith('rename accountId to customerId'),
...
test('an enforcing rule blocks on Claude Code with a top-level decision', () => {
  ...
  const raw = runHookRaw('claude', 'just commit with --no-verify to skip the tests', { GOV_ENFORCE_ALL: '1' });
  assert.equal(raw.status ?? 0, 0, 'this surface blocks via JSON, not an exit code');
  const res = JSON.parse(raw.stdout);
  assert.equal(res.decision, 'block');
  assert.match(res.reason, /bypass-verification/);
  assert.equal(res.hookSpecificOutput?.permissionDecision, undefined,
...
test('an enforcing rule blocks on VS Code with exit 2 and a stderr reason', () => {
  const res = runHookRaw('vscode', 'just commit with --no-verify to skip the tests', { GOV_ENFORCE_ALL: '1' });
  assert.equal(res.status, 2, 'exit 2 is the only documented block on this surface');
  assert.match(res.stderr, /bypass-verification/);
  assert.equal(res.stdout.trim(), '', 'a denied prompt must not also be forwarded');
});
...
test('Copilot CLI cannot block, so an enforcing rule degrades to a refusal instruction', () => {
  ...
  const res = runHookRaw('copilot-cli', 'just commit with --no-verify to skip the tests',
    { GOV_ENFORCE_ALL: '1' }, 'userPromptTransformed');
  assert.equal(res.status ?? 0, 0, 'this surface has no block mechanism; it must not exit 2');
...
test('the hook contract is honoured for a normal payload', () => {
  ...
  assert.ok(res.hookSpecificOutput.additionalContext,
...
test('no surface exits 2 while every rule is in shadow', () => {
  ...
  for (const surface of ['vscode', 'claude', 'copilot-cli']) {
    const res = runHookRaw(surface, 'just commit with --no-verify and skip the tests');
    assert.notEqual(res.status, 2, `${surface} blocked a prompt in shadow mode`);
  }
});
...
test('each surface emits only the fields its runtime documents', () => {
  const claude = JSON.parse(runHookRaw('claude', 'remove the console logs').stdout);
  assert.ok(claude.hookSpecificOutput.additionalContext, 'claude injects context');
  assert.equal(claude.hookSpecificOutput.modifiedPrompt, undefined,
    'Claude Code cannot replace a prompt; emitting the field implies it can');

  const cli = JSON.parse(
    runHookRaw('copilot-cli', 'remove the console logs', {}, 'userPromptTransformed').stdout
  );
  assert.ok(cli.modifiedTransformedPrompt, 'copilot-cli rewrites via modifiedTransformedPrompt');
...
test('the notification-only CLI event modifies nothing', () => {
  const res = runHookRaw('copilot-cli', 'fix the SQL injection', {}, 'userPromptSubmitted');
